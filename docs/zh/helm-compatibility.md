...
基准输出测试结果会在每次 push 和 pull request 中通过 [CI workflow](https://github.com/GaTTGeng/HelmSharp/blob/master/.github/workflows/ci.yml) 验证。CI runner 会在 .NET SDK 旁安装 Helm CLI（`v3.12.3`），并执行完整的基准输出测试套件，包括测试用 Chart 和公开 Chart 对比。JSON 报告会作为每次 workflow run 的 artifacts 发布。
