...
GitOps 系统需要确定性的文件和可 review 的 diff，而不是直接修改集群。`HelmTemplateRenderer` 负责产出应提交的 manifest。

## 生产环境注意事项

- 生成路径保持稳定。
- 如果 reviewer 需要理解输出变化，请把 values 一并提交。
- 对关键内部 Chart 增加自己的 golden test。

## 下一步

阅读 [Values](../guide/values.md)，明确环境覆盖策略。
